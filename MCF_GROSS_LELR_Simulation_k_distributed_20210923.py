'''
Using k-distributed sea clutter to test the three trackers
(MKCF-TBD, MSAR-TBD, WTSA-TBD)

Compare three algorithms of tracking in the simulated clutter:
MCF(short of MKCF-TBD), Grossi(MSAR-TBD in TAES paper) and LELR(WTSA-TBD)
with same input frames and same tracker Framework.
This file is based on
'MCF_TBD_Param_Test_Simulated_3targets_20210208.py'
'DP_TBD_Grossi_ETTsim_20201229.py'
'DP_TBD_LELR_ETTsim_20210304.py'

Test the MCF's gt_tracker only(Initiate only the gt targets)
to monitoring the status and merit value changes across the frame.
'''
...
import  motion_simulation_k_distribution_20210923 as msim_model
...
class MCF_TBD_SIM(): #MCF_TBD Tracker implemented for simulated situations.
    def __init__(self,img_w, img_h, start_fid, end_fid,
        ...
        self.number_of_alarm_list    = []
        self.size_of_alarm_list      = []
        if (self.bVerbose):
...
class DP_TBD_SIM(): #Grossi_DP_TBD Tracker and LELR_DP_TBD are implemented for simulated situations.
    ...
    def activate(self,frame, frame_no, sw_type, snr):
        ...
        if frame_no >= (self.dp_tbd.L - 1):  # fid start from 0
            ...
            self.dp_tbd.confirm_trajectory(frame_no, self.dp_tbd.nodes_dict, self.dp_tbd.gamma2, bshow_fkl=False)
        time_cost_per_frame = time.perf_counter() - time_cost_per_frame
        self.time_counter_list.append(time_cost_per_frame)
