...
import K_distributed_SeaClutter_Simulation_20210919 as kclutter_model
...
def add_gaussian_template_on_clutter(cx, cy, w, h, theta, erc, snr, clutter_background, swerling_type=0):
    ...
    # Get decreasing_coeffient to make sure the inner gaussian template satisfy the snr requirement.
    tcx, tcy = w_t/2, h_t/2
...
def get_frame(img_w, img_h, frame_no, snr, gt_dict, swerling_type=0):
    ...
    #k_background   = np.array(Image.open('%s/correlated_k_decay_Gaussian_speckle/%2d.tif' % (background_dir, frame_no+1)))


    k_background, _ = kclutter_model.generate_K_distributed_noise(gamma_shape=5)
    k_background    = k_background.astype(np.float32)
    background = k_background

    # Erc: average clutter energy.
    erc = np.sum(background ** 2) / background.size
    #add targets on the simulated position in each frame
    simulated_frame = background
