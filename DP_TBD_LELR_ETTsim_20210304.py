'''
In this file, LELR scoring function is adopted for the DP_TBD algorithm
which is based on GrossiTSP2013 algorithms.
In 'Jiang Haichao TAES2017' paper, fluctuating extended targets of Swerling target type 0,1,3
are discussed using different scoring function to implement the DP_TBD.
The LELR of Spread Point Function (SPF) based extended targets is
expressed as weighted square amplitudes. Different Swerling types use varied weight equations.

This file is based on 'DP_TBD_Grossi_ETTxxx.py'.
Created by Yi Zhou@Provence-Dalian on 2021-03-04
'''
...
import csv
from matplotlib.patches import Rectangle
#import two_dimensional_gaussian_fit_20210304 as gffit_model  #fit data to get coefficients.
...
class Nodes():
    def __init__(self):
        ...
        self.am2  = 0       # average echo square of intensity amplitude in a plot
...
class DP_TBD_LELR_Grossi():
    '''
    Implementation of the Grossi's Dynamic programming in paper Grossi_TSP2013.
    For extended target, Gaussian spread point function is used for each extended target.
    Here we fit the intensity of the input data to a 2d gaussian without rotation for sake of simplicity of computing.
    Due to the different merit function of swerling type 0, 1, 2,
    the target type should be given at the initial step.
    '''
    def __init__(self, P=4, Q=3, L=10, gamma2 = 4, sigma_n = 1, swerling_type=0):
        ...
        self.sigma_n = 1  # average clutter power. Should be updated in each frame.
        self.swerling_type = swerling_type #swerling target type for SPF extended target
    ...
    def get_lelr_value(self, blobroi, swerling_type):
        '''
        Comput the Logrithm envelope likelihood ratio, based on the eq(40-43)of JiangTAES2017 paper.
        :param frame:
        :param bloblist:
        :return:
        '''
        bh, bw = blobroi.shape[:2]

        spf_sigma = 2 #gaussian spread function's sigma
        grid_y = np.arange(bh) - int(bh / 2)
        grid_x = np.arange(bw) - int(bw / 2)
        rs, cs = np.meshgrid(grid_x, grid_y)
        hmat = np.exp(-0.5 / spf_sigma ** 2 * (rs ** 2 + cs ** 2))
        hmat = hmat/np.sum(hmat)
        sigma_t  = np.sum((blobroi-np.sqrt(self.sigma_n))**2)/blobroi.size
        # params   = gffit_model.fitgaussian(blobroi)
        # width_x  = params[3]
        # height_y = params[4]
        #
        # if width_x>50 or height_y>50: #uniform distribution:
        #     hmat = np.ones_like(blobroi)
        # else: #reasonable Gaussian distribution
        #     hmat = np.zeros_like(blobroi) # coefficient matrix
        #     hgaussian= msim_model.gaussian_kernel2d(sigma_x=width_x/2, sigma_y=height_y/2, theta=0)
        #     gh, gw   = hgaussian.shape[:2]
        #     bh, bw   = blobroi.shape[:2]
        #     #crop the hgaussian coefficient matrix to get the same width and height as blobroi.
        #     if gw >= bw: #Fitted Gaussian spread, when it wider than blobroi
        #        tlx_orig = int(gw/2) - int(bw/2)
        #        tlx_hmat = 0
        #        crop_w   = bw
        #     else:
        #        tlx_orig = 0
        #        tlx_hmat = int(bw/2) - int(gw/2)
        #        crop_w   = gw
        #
        #     if gh >= bh: #Fitted Gaussian spread wider than blobroi
        #        tly_orig = int(gh/2) - int(bh/2)
        #        tly_hmat = 0
        #        crop_h   = bh
        #     else:
        #        tly_orig = 0
        #        tly_hmat = int(bh/2) - int(gh/2)
        #        crop_h   = gh
        #     hmat[tly_hmat:tly_hmat+crop_h, tlx_hmat:tlx_hmat+crop_w] = \
        #         hgaussian[tly_orig:tly_orig+crop_h, tlx_orig:tlx_orig+crop_w]
        #     hmat = hmat/np.sum(hmat)

        if swerling_type == 0:
            #Eq(40) of Jiang's paper
            weights = hmat**2
        if swerling_type == 1:
            #Eq(41)
            tp = (hmat**2)*sigma_t
            weights = tp/(tp+self.sigma_n)
        if swerling_type==3: #Eq(42)
            tp = (hmat**2)*sigma_t
            weights = tp/(tp+2*self.sigma_n)
            #weights = hmat**2
        #weights = hmat ** 2
        #weights = weights/weights.size
        lelr_score_value = np.sum(weights*(blobroi**2)) # eq(43)
        # if(lelr_score_value>1000):
        #     print('')
        return lelr_score_value
    ...
    def generate_nodes(self, frame, fid, bloblist, timestep=1):
        ...
        for blobrect in bloblist:
            ...
            new_node.wh   = (w,h)
            new_node.am2   = np.average(blobroi**2)
            #new_node.val  = new_node.am**2
            new_node.val  = self.get_lelr_value(blobroi, self.swerling_type)
            # print('%s mean square amplitude %.5f, LELR %.5f, SNR %.2f'
            #      %(new_node.name, np.average(blobroi**2), new_node.val, 10*np.log10(new_node.am2/2)))
        ...
        if len(self.fidlist)>1:
            ...
            for nd_pre in nodes_pre:
                if len(nd_pre.nxt_list)==0: # no linked next nodes
                    ...
                    zero_node.val = 0  # for LELR, due to log(1)=0
            ...
            for nd in nodes_prunlist:
                ...
                try:
                    while nd.tau[l] == maxnode.tau[l]:  # nd share the same roots with maxnode
                        ...
                        zero_node.val = 0 #for lelr. 10log1=0
    ...
    def confirm_trajectory(self, lastfid, nodes_dict, gamma2=4, bshow_fkl=False):
        ...
        for nd in lastfid_nodes_list:
            if len(nd.tau) >= self.L: # integration should be enough
                ...
                for i in range(-1,-(self.L+1),-1):
                    fkl       += nd.tau[i].val
                    ave_power += nd.tau[i].am2
                    str_traj_am = str_traj_am + '%.1f '%(nd.tau[i].am2)
                ave_power = ave_power/self.L
                nd.fkl = fkl
                # if(bshow_fkl):
                #     print('Candidate %s traj\'s length %d - FKL %.2f snr %.2f'
                #           %(nd.name, len(nd.tau),nd.fkl, 10*np.log10(ave_power/2)))
                ...
                if nd.fkl > gamma2 and self.count_consecutive_zero_nodes(nd.tau) <= self.P\
                    ...
                    # print('Confirmed %s traj\'s length %d in frame %d - FKL %.2f snr %.2f'
        ...
        for tid in traj_dict:
            ...
            for id,nd in enumerate(tau):
                ...
                if nd.fid == fid: #draw current rectangle on this fid
                    rect_patch = Rectangle(xy=[x-w/2, y-h/2], width=w, height=h, angle=0, color=color_tuple,fill=None)
...
#def draw_bounding_boxs(frame, bloblist, ax, color=(255,255,0)):
def draw_bounding_boxs(frame, bloblist, ax, color=(255, 255, 0), offset_xy=(0, 0)):
    ...
    for i,blob in enumerate(bloblist):
        ...
        ax.text(tlx, tly, 'B%03d'%(i+1), color=(1, 1, 1, 1), fontsize=6)
        ax.add_artist(rect)
    #plt.show()


def test_dp_tbd_verbose(integrated_frames=10, gamma2=1.9, snr=10, swerling_type=0, kval=1.3):
    '''
    Test dynamic tbd by tracking the simulated multiple extended targets.
    :return:
    '''
    '''
    ...
    dp_tbd = DP_TBD_LELR_Grossi(P=4, Q=3, L=integrated_frames, gamma2=gamma2, sigma_n=1,
                                swerling_type=swerling_type)  # default gamma2=4
    fig, ax = plt.subplots()
    fid_cfar_list = []
    pfa_cfar_list = []
    pd_cfar_list = []
    ...
    for fid in range(frame_nums):
        ...
        frame = msim_model.get_frame(img_w, img_h, fid, snr, gt_rotate_rect_trajectory_dict)  # gray_scale image
        # uframe = uti.frame_normalize(frame) * 255
        # uframe = uframe.astype(np.uint8)
        ...
        print('Frame %02d pfa_cfar %.5f, pfa_seg %.5f, pd_cfar %.5f, pd_seg %.5f' % (
        fid, pfa_cfar, pfa_seg, pd_cfar, pd_seg))
    ...
    _, _, res_table = eval_model.print_metrics(precision_dict, false_alarm_acc, img_w, img_h, frame_nums,
                                               overall_roh_dict)
    ...
    record = [swerling_type, snr, np.mean(time_counter_list), np.mean(pfa_cfar_list), np.mean(pfa_seg_list),
              np.mean(pfa_trk_list),
              np.mean(pd_cfar_list), np.mean(pd_seg_list), np.mean(pd_trk_list),
...
def test_dp_tbd_parameter(integrated_frames, gamma2=1.9, snr=10, swerling_type=0):
    '''
    Check parameters of the dynamic tbd by tracking the simulated multiple extended targets.
    # snr means signal to noise ratio in the simulated clutter background.
    :return:
    ...
    dp_tbd = DP_TBD_LELR_Grossi(P=4, Q=3, L=integrated_frames, gamma2 = gamma2, sigma_n=1, swerling_type=swerling_type)#default gamma2=4
    #fig, ax = plt.subplots()
    fid_cfar_list     = []
    pfa_cfar_list     = []
    pd_cfar_list      = []
    ...
    # fid_trk_list, pfa_trk_list, pd_trk_list = sp_model.get_pfa_pd_via_trajectory_rrect_v2(frame_track_trajectory,
    #                                                                              gt_trajecotry_dict, precision_dict,
    #                                                                              frame_height = img_h, frame_width=img_w,
    #                                                                              bshrink_tbox=False)
...
if __name__ == '__main__':
    test_dp_tbd_verbose(integrated_frames=9, gamma2=0.2, snr=0, swerling_type=1, kval=1.3)
    ...
    # name of csv file
    filename = "/Users/yizhou/code/taes2021/results/DP_TBD_LELR_SIMULATE.csv"
    # writing to csv file
    with open(filename, 'w') as csvfile:
        ...
        for  sw_type in [0, 1, 3]:
            for snr in snrs:
                record = test_dp_tbd_parameter(integrated_frames=9, gamma2 = 1, snr=snr, swerling_type=sw_type)
                # writing the data rows
                csvwriter.writerow(record)
                csvfile.flush()
    print('Parameter_test complete! Remember to modify the name of csv for saving records!')

    #test_dp_tbd_verbose(integrated_frames=10, gamma2=1.9, snr=10, swerling_type=0)
    #plt.show()